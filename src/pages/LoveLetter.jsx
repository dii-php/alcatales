...
function SongCard({ songId, songTitle, songThumbnail, channelTitle, compact = false }) {
  ...
  return (
    <div style={{ borderRadius: compact ? 10 : 14, overflow: 'hidden', background: 'var(--color-surface2)', border: '1px solid var(--color-border)', marginBottom: compact ? 10 : 16 }}>
      {compact ? (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px' }}>
          {songThumbnail
            ? <img src={songThumbnail} alt={songTitle} style={{ width: 44, height: 33, borderRadius: 6, objectFit: 'cover', flexShrink: 0 }} />
            : <div style={{ width: 44, height: 33, borderRadius: 6, background: '#1a1a2e', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><Music size={14} color="#aaa" /></div>
          }
...
export default function LoveLetter() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (modalMode === 'add') {
        // Single call — get docRef for real ID
  ...
  return (
    <div style={{ minHeight: '100vh', paddingTop: 64 }}>
      <div style={{ background: 'var(--gradient-hero)', padding: '60px 24px 72px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        {[...Array(3)].map((_, i) => (
          <Heart key={i} size={14 + i * 5} fill="rgba(255,255,255,0.3)" color="rgba(255,255,255,0.3)"
            style={{ position: 'absolute', top: `${18 + i * 22}%`, left: `${8 + i * 22}%`, animation: `floatHeart ${2.2 + i * 0.6}s ease-in-out infinite`, animationDelay: `${i * 0.3}s`, pointerEvents: 'none' }} />
        ))}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 18 }}>
              {pagedLetters.map(letter => (
                <div key={letter.id} className="letter-card" onClick={() => setOpen(letter)}>
                  {letter.songId ? (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 11px', background: 'var(--color-surface2)', borderRadius: 10, marginBottom: 12, border: '1px solid var(--color-border)' }}>
                      {letter.songThumbnail
                        ? <img src={letter.songThumbnail} alt={letter.songTitle} style={{ width: 40, height: 30, borderRadius: 5, objectFit: 'cover', flexShrink: 0 }} />
                        : <div style={{ width: 40, height: 30, borderRadius: 5, background: 'var(--color-border)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><Music size={13} color="var(--color-text-muted)" /></div>
                      }
                      <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      {isAdmin && (
                        <div style={{ display: 'flex', gap: 4, flexShrink: 0, marginLeft: 4 }}>
                          <button onClick={e => { e.stopPropagation(); openEdit(letter); }} className="letter-card__action-btn letter-card__action-btn--edit"><Pencil size={11} /></button>
                          <button onClick={e => { e.stopPropagation(); handleDelete(letter.id); }} className="letter-card__action-btn letter-card__action-btn--del"><Trash2 size={11} /></button>
                        </div>
                      )}
                    </div>
                  ) : (
                    isAdmin && (
                      <div className="letter-card__actions">
                        <button onClick={e => { e.stopPropagation(); openEdit(letter); }} className="letter-card__action-btn letter-card__action-btn--edit"><Pencil size={12} /></button>
                        <button onClick={e => { e.stopPropagation(); handleDelete(letter.id); }} className="letter-card__action-btn letter-card__action-btn--del"><Trash2 size={12} /></button>
                      </div>
                    )
                  )}
                  ...
                  <p style={{ fontSize: 11, color: 'var(--color-text-muted)', marginBottom: 10, opacity: 0.7, fontStyle: 'italic' }}>{formatDate(letter.createdAt)}</p>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    <button onClick={e => { e.stopPropagation(); navigate(`/love-letter/${letter.id}`); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-primary)', padding: 4, display: 'flex', alignItems: 'center', gap: 4, fontSize: 11 }}>
      ...
      {open && (
        <div className="modal-overlay" onClick={() => setOpen(null)}>
          <div className="modal-box" style={{ maxWidth: 580 }} onClick={e => e.stopPropagation()}>
            ...
            <button onClick={() => navigate(`/love-letter/${open.id}`)} style={{ position: 'absolute', top: 14, right: 46, background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-text-muted)' }} title="Buka halaman penuh"><ExternalLink size={18} /></button>
            ...
            {open.songId && <SongCard songId={open.songId} songTitle={open.songTitle} songThumbnail={open.songThumbnail} channelTitle={open.channelTitle} compact={false} />}
            <div style={{ background: 'var(--color-surface2)', borderRadius: 12, padding: 18, fontFamily: 'Playfair Display', fontStyle: 'italic', lineHeight: 1.8, fontSize: 15, color: 'var(--color-text)', whiteSpace: 'pre-wrap', maxHeight: '40vh', overflowY: 'auto' }}>
              {open.content}
            </div>
          </div>
        </div>
        ...
        <div className="modal-overlay" onClick={closeModal}>
          <div className="modal-box" style={{ maxWidth: 520 }} onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div>
                <label style={{ display: 'flex', alignItems: 'center', gap: 6 }}><Music size={13} /> Lagu (opsional)</label>
                <YouTubeSearch value={form.song} onChange={song => setForm(f => ({ ...f, song }))} placeholder="Cari lagu di YouTube..." />
              </div>
      ...
      <style>{`
        .letter-card { background:var(--color-surface); border-radius:16px; padding:18px; box-shadow:var(--card-shadow); border:1px solid var(--color-border); cursor:pointer; position:relative; transform:translateZ(0); transition:transform 0.18s ease; }
        .letter-card:hover { transform:translateY(-4px); }
        .letter-card__actions { position:absolute; top:12px; right:12px; display:flex; gap:4px; opacity:0; transition:opacity 0.15s; z-index:2; }
        .letter-card:hover .letter-card__actions { opacity:1; }
        .letter-card__action-btn { width:26px; height:26px; border-radius:6px; border:none; cursor:pointer; display:flex; align-items:center; justify-content:center; }
        ...
        @media(max-width:480px){ .letter-card__actions{opacity:1;} }
      `}</style>
    </div>
  );
}
