...
export default function Countdown() {
  // serverNowMs = real-world "now" fetched from NTP API
  const serverNowRef = useRef(null);
  // performance.now() at the moment we got the server time
  ...
  useEffect(() => {
    const id = setInterval(() => {
      ...
      // Compute current real time: server time + elapsed wall-clock time since fetch
  ...
  if (!elapsed) return (
    <div style={{ color: 'rgba(255,255,255,0.6)', fontSize: 14 }}>Memuat waktu...</div>
  );
      ...
      {units.map((u, i) => (
        <React.Fragment key={u.label}>
          <div style={{
            ...
            textAlign: 'center', minWidth: isMobile ? 64 : 90,
            border: '1px solid rgba(255,255,255,0.18)',
          }}>
            <div style={{ fontSize: isMobile ? 28 : 42, fontWeight: 700, color: 'white', fontFamily: 'Inter', letterSpacing: '-0.02em', lineHeight: 1 }}>
              {u.value}
            </div>
          ...
          {i < 3 && <span style={{ fontSize: isMobile ? 20 : 36, fontWeight: 700, color: 'rgba(255,255,255,0.7)', marginTop: isMobile ? -10 : -16, flexShrink: 0 }}>:</span>}
        </React.Fragment>
      ))}
    </div>
  );
