// api/unsubscribe.js — delete subscriber doc via token (called from email link)
...
export default async function handler(req, res) {
  ...
  if (!token) return res.status(400).send(html('Token tidak valid', 'Token unsubscribe tidak ditemukan.', false));

  try {
    initAdmin();
    const db = getFirestore();
    const snap = await db.collection('subscribers').where('token', '==', token).limit(1).get();

    if (snap.empty) return res.status(404).send(html('Tidak Ditemukan', 'Token tidak valid atau email sudah di-unsubscribe.', false));

    // DELETE doc entirely
    await snap.docs[0].ref.delete();

    return res.status(200).send(html('Berhasil Unsubscribe', 'Kamu tidak akan menerima notifikasi dari ALCA lagi. Terima kasih sudah bersama kami 💕', true));
  } catch (err) {
...
function html(title, message, success) {
  return `<!DOCTYPE html>
<html lang="id">
<head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>${title} — ALCA</title>
<style>
